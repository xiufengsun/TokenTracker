...
function readExact(fd, target, offset) {
  ...
  while (read < target.length) {
    ...
    if (!count) throw error("database or WAL was truncated during the read; retry when TRAE is idle.");
    read += count;
  }
  return target;
}
...
function indexWal(fd, size, pageSize) {
  ...
  if (size < 32) throw error("incomplete WAL header; retry when TRAE is idle.");
  ...
  for (let offset = 32; offset + frame.length <= size; offset += frame.length) {
    ...
    if (sum[0] !== frame.readUInt32BE(16) || sum[1] !== frame.readUInt32BE(20)) throw error("WAL frame checksum failed; retry when TRAE is idle.");
...
function openTraeSnapshot(dbPath, env = process.env) {
  ...
  try {
    ...
    handles.push(dbFd);
    if (!sameStat(before[0], fs.fstatSync(dbFd, { bigint: true }))) throw error("database changed while opening; retry when TRAE is idle.");
    let walFd = null;
    if (before[1]) {
      walFd = fs.openSync(paths[1], "r");
      handles.push(walFd);
      if (!sameStat(before[1], fs.fstatSync(walFd, { bigint: true }))) throw error("WAL changed while opening; retry when TRAE is idle.");
    }
