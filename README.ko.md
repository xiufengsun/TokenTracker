...
| **Command Code** | ✅ 자동 | 패시브 세션 로그 리더 (`~/.commandcode/projects/<cwd-slug>/<session-id>.jsonl`, `*.checkpoints.jsonl` 스냅샷은 건너뜀). 세션 로그를 로컬에서 스캔하고 토큰 수와 CLI가 기록한 `costUsd` 등 필요한 사용량 메타데이터를 추출합니다. 캐시를 포함한 input에서 캐시 읽기와 쓰기 토큰을 모두 빼 이중 계산을 방지합니다. Token Tracker는 프롬프트·응답·코드 본문을 저장하거나 업로드하지 않습니다. 디렉터리는 `TOKENTRACKER_COMMANDCODE_HOME`으로 변경 가능 |
